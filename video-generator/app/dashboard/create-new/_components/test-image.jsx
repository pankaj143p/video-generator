...
export default function ImageTest({ onImageUpload }) { // Accept the onImageUpload prop
  const [selectedFile, setSelectedFile] = useState(null);
  const [uploadedUrl, setUploadedUrl] = useState(null);
  const [loading, setLoading] = useState(false);
  const [error, setError] = useState(null);
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const result = await response.json();
      if (response.ok) {
        const url = `https://storage.googleapis.com/YOUR-FIREBASE-BUCKET/uploaded-images/unique-image-id.jpg`;
        setUploadedUrl(url);
        onImageUpload(url); // Call the onImageUpload function to update the main page state
        setError(null);
      } else {
        setError(result.message || 'Something went wrong');
      }
    ...
    <div>
      ...
      {uploadedUrl && (
        <div>
          <p>Image uploaded successfully! View it below:</p>
          <img src={uploadedUrl} alt="Uploaded" style={{ width: '300px', marginTop: '10px' }} />
          <a href={uploadedUrl} target="_blank" rel="noopener noreferrer">Open in new tab</a>
        </div>
      )}
    </div>
  );
